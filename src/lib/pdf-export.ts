...
/** Strip characters Helvetica cannot encode in pdf-lib. */
...
export function userRequestedPdf(text: string): boolean {
  return (
    /\b(generate|create|make|export|download|turn\s+this\s+into)\b[\s\S]{0,40}\bpdf\b/i.test(
      text
    ) || /\bpdf\b[\s\S]{0,40}\b(generate|create|make|export|download)\b/i.test(text)
  );
}

export function pdfFilename(text: string): string {
  const slug = text
    .slice(0, 40)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
